...
def gainspector(**kw):
    ...
    # complex gains
    try:
        G = xds_from_zarr("::".join(opts.complex_gain.rsplit("/", 1)), chunks=None)
    except:
        raise ValueError(f"Failed to load complex gains at {opts.complex_gain}")
    # net gains
    try:
        N = xds_from_zarr("::".join(opts.net_gain.rsplit("/", 1)), chunks=None)
    except:
            ...
            for c in [0, -1]:
                for i, ax in enumerate(axs.ravel()):
                    if i < nant:
                        ...
                        im = ax.imshow(
                            g, cmap="inferno", interpolation=None, vmin=glow, vmax=ghigh
                        )
                ...
                plt.savefig(
                    opts.output_filename + f"_corr{c}_scan{s}_{name}.png",
                    dpi=100,
                    bbox_inches="tight",
                )
                plt.close()
...
def plot_gains(gds, nds, ref_ant, p, c):
    ...
    # real
    fig, axs = plt.subplots(nrows=1, ncols=2, figsize=(flength, 2 * tlength))
